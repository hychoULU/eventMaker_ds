...
- ShowDecisionReward는 suffix 없이 저장하며, DecisionEnd Node에 연결된 Choice에서만 사용할 수 있다.
...
4. 
비주얼 이벤트 에디터(Visual Event Editor) 시스템 명세서 (v3.4.0 기준)

1) 데이터 구조 및 논리 규칙

이벤트(Event): 각 이벤트는 고유의 시작 노드(Depth 0)를 가지며, 생성 시 시작 노드와 기본 선택지 1개를 함께 만든다.
v3.4.0 기준, Fixed / Random / Npc / Tutorial / Decision 총 5타입의 이벤트를 가진다.
노드(Node):
...
Decision 보상 툴팁(ShowDecisionReward):
ActiveTooltipType을 ShowDecisionReward로 설정하면 웹 툴에서는 ShowAction과 동일하게 해당 Choice의 OnSelectAction을 보여준다.
이 타입은 DecisionEnd Node에 포함된 Choice에서만 선택 가능하다.
Choice가 포함된 부모 Node의 NodeType이 DecisionEnd에서 다른 타입으로 변경되거나, 불러오기/임포트 중 잘못된 위치에서 발견되면 ActiveTooltipType을 ShowAction으로 정규화한다.

우클릭 확장 메뉴(Context Menu):

이벤트 / 노드 / 선택지 우클릭 시 전용 메뉴가 나타난다.
이벤트 메뉴에서는 Copy Event / Paste Event를 지원한다.
선택지 메뉴에서는 툴팁 타입(Self, Decision Reward, Choice, Probability, None)을 즉시 변경할 수 있다. 단, Decision Reward는 DecisionEnd Node의 Choice에서만 노출된다.
